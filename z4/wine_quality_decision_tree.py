...
"""
==========================================
Decision Tree to classify dataset winequality-white.txt
Creators:
...
def visualize_classifier(classifier, X, y, title=''):
    ...
    plt.yticks((np.arange(int(X[:, 1].min() - 1), int(X[:, 1].max() + 1), 10.0)))


data = np.loadtxt('winequality-white.txt', delimiter=';')
X, y = data[:, :2], data[:, -1]
...
class_names = ['No diabetics', 'Diabetics']
print("\n" + "#" * 40)
