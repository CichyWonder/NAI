...
class Oware(TwoPlayerGame):
    ...
    def __init__(self, players):
        ...
        self.current_player = 1  # Gracz relany zaczyna
    ...
    def possible_moves(self):
        ...
        if self.current_player == 1:
            if max(self.board[:6]) == 0: return ['None']  # Ruchy Gracza
            moves = [i for i in range(6) if (self.board[i] >= 6 - i)]  # a b c d e f
            if moves == []:  # if no moves are possible
                moves = [i for i in range(6) if self.board[i] != 0]  # jeżeli e. g. e is 0, nie pozwól na ruch
        else:
            if max(self.board[6:]) == 0: return ['None']  # Gracz AI
    ...
    def is_over(self):
        ...
        - Gracz nie może wykonać kolejnego ruchu (Uschnął)
        """
